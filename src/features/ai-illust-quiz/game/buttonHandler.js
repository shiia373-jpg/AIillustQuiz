...
async function handleButton(interaction) {
  ...
  if (interaction.customId === 'quiz_input_word') {
    ...
    if (!game) return interaction.reply({ content: 'まず /start-quiz でクイズを設定してください。', flags: 64 });
    if (interaction.user.id !== game.quizmasterId) return interaction.reply({ content: '出題者のみ操作できます。', flags: 64 });
  ...
  if (interaction.customId === 'quiz_start') {
    const game = getGame(guildId);
    if (!game) return interaction.reply({ content: 'まず /start-quiz でクイズを設定してください。', flags: 64 });
    if (game.active) return interaction.reply({ content: 'すでにクイズが進行中です。', flags: 64 });
    if (interaction.user.id !== game.quizmasterId) return interaction.reply({ content: '出題者のみ操作できます。', flags: 64 });
  ...
  if (interaction.customId === 'quiz_image_approve') {
    ...
    if (!game || !game.pendingImageBuffer) {
      return interaction.reply({ content: '⚠️ 確認待ちのイラストがありません。', flags: 64 });
    }
  ...
  if (interaction.customId === 'quiz_image_reject') {
    const game = getGame(guildId);
    if (!game) return interaction.reply({ content: '⚠️ エラーが発生しました。', flags: 64 });
    if (interaction.user.id !== game.quizmasterId) {
      return interaction.reply({ content: '出題者のみ操作できます。', flags: 64 });
    }
  ...
  const game = getGame(guildId);
  if (!game || !game.active) return interaction.reply({ content: '進行中のクイズがありません。', flags: 64 });
  const isQuizmaster = interaction.user.id === game.quizmasterId;

  if (interaction.customId === 'quiz_add_hint') {
    if (!isQuizmaster) return interaction.reply({ content: '出題者のみ操作できます。', flags: 64 });
  ...
  if (interaction.customId === 'quiz_answer') {
    if (isQuizmaster) return interaction.reply({ content: '出題者は回答できません！', flags: 64 });
  ...
  if (interaction.customId === 'quiz_check_winners') {
    if (!isQuizmaster) return interaction.reply({ content: '出題者のみ確認できます。', flags: 64 });

    const winners = game.roundWinners || [];
    if (winners.length === 0) {
      return interaction.reply({ content: '📋 まだ誰も正解していません。', flags: 64 });
    }
